...
def FillHole_RGB(im_th):
    ...
    # 函数findContours获取轮廓
    contours, hierarchy = cv.findContours(im_floodfill_inv_copy,
                                           cv.RETR_TREE,
    ...
    for i in range(len(contours)):
        ...
        if (area < 100):
            ...
            cv.drawContours(im_floodfill_inv_copy, c_min, -1, (255, 255, 255),
                            thickness=-1)
            continue

    # 把im_in、im_floodfill_inv这两幅图像结合起来得到前景
    im_out = im_th | im_floodfill_inv_copy
    # im_out = im_floodfill
...
if __name__ == '__main__':
    ...
    # imgpXY, imgpS = Prewaitt(H, W, imgN1)
    # imgXandY, imgabS = Sobel(H, W, imgN1)
    # imgXYH1, imgXYH2, imgXYH3, imgXYH4 = Synthetic(H,W,imgN1)
    # imgXYH1, imgXYH2 = Laplace(H,W,imgN1)
    #
    ...
    # erode_img = getErode(imgR, kernelE)
    # cv.imshow("Erode", erode_img)

    imgErZhihua = erzhihua(dilate_img)

    # fill_img = fillHole(imgErZhihua)
    fill_img = FillHole_RGB(imgErZhihua)
    cv.imshow("Fill1",fill_img)

    # erode_img = getErode(fill_img,kernelE)
    # cv.imshow("Erode",erode_img)

    # draw(fill_img)
   #
   # #轮廓检测
   #  contours, hierarchy = cv.findContours(fill_img, cv.RETR_TREE,
